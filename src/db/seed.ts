  ...
  for (const [symbol, data] of Object.entries(MOCK_STOCKS_DATABASE)) {
    ...
    await db
      ...
      .values({
        ...
        macdSignal: '1.12',
        macdHistogram: '0.33',
        bollingerUpper: String(data.ceilingPrice != null ? data.ceilingPrice * 0.97 : data.price * 1.05),
        bollingerMiddle: String(data.price),
        bollingerLower: String(data.floorPrice != null ? data.floorPrice * 1.03 : data.price * 0.95),
    ...
    await db
      ...
      .values({
        ...
        pb: String(data.pb ?? ''),
        roe: String(data.roe ?? ''),
        eps: String(data.pe ? Math.round(data.price / data.pe) : ''),
        bvps: String(data.pb ? Math.round(data.price / data.pb) : ''),
      })
      .onConflictDoNothing();
    ...
    await db
      ...
      .values({
        ...
        upsidePercent: String(data.fairValue != null && data.price > 0 ? ((data.fairValue - data.price) / data.price * 100).toFixed(2) : ''),
        rating: data.fairValue != null ? (data.fairValue > data.price ? 'UNDERVALUED' : 'FAIR') : 'UNRATED',
        marginOfSafety: '15.0',
